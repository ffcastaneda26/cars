                                ...
                                <tr>
                                    ...
                                    <td class="border leading-tight font-semibold text-gray-900 hover:underline px-2 py-1 text-center w-32">
                                        @if($record->hasrole($role->id))
                                            <button wire:click="unlinkRecord({{ $record->id }})" class="bg-red-500 hover:bg-red-700 text-white font-bold py-1 px-2 rounded text-center">{{__("Remove")}}</button>
                                        @else
                                            <button wire:click="linkRecord({{ $record->id }})" class="bg-indigo-500 hover:bg-indigo-900 text-white font-bold py-1 px-2 rounded text-center">{{__("To Assign")}}</button>
                                        @endif
                                    </td>
                                </tr>
                                @endforeach
